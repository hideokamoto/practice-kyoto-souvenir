...
## 12. 実装同期メモ（2026-07 UI/デザインリニューアル）

本ドキュメントの初版と実装の間に乖離が生じていたため、Apple HIG デザインスキルに基づくリニューアルで
以下のとおり整合を取り直した。以降は本節を実装のソース・オブ・トゥルースとする。

### 12.1 発見ページの構造（確定版）

「今日、何をしよう？」に対し、京都の体験は **「どこへ行くか（観光地）」と「何を持ち帰るか（お土産）」**という
性格の異なる 2 つの問いに分かれる。両者は代替関係ではなく補完関係のため、同一画面に 2 セクションで併存させる。
ただし各セクションは常に **1 件だけ**を主役として提示し、選択肢過多を避ける。
...
├─────────────────────────┤
│ 挨拶（日付・京都）        │
│ 絞り込みチップ            │ ← すべて / 未訪問 / お気に入り（実データで機能するもののみ）
├─────────────────────────┤
│ おすすめスポット  [別の提案]│ ← セクション見出し右に引き直しアクション
│  ┌───────────────────┐   │
│  │ ヒーローカード(観光地・1件)│   │
│  └───────────────────┘   │
├─────────────────────────┤
...
│  └───────────────────┘   │
├─────────────────────────┤
│ 探索モード（折りたたみ）   │ ← 検索・リストは能動的探索者のための二次動線
└─────────────────────────┘
```

### 12.2 初版からの変更点

- **「別の提案」ボタンを復活**：各セクション見出しの右に配置し、そのセクションだけを引き直す
  （`shuffleSight()` / `shuffleSouvenir()`）。初版にあった全体シャッフル FAB は、機能重複と
  Material 由来パターンのため撤去した
- **絞り込みチップの整理**：「寺社」「庭園」は `Sight` 型にカテゴリ列がなく非機能だったため撤去。
  実データで機能する「すべて / 未訪問 / お気に入り」のみ残し、`activeFilter` を実際に提案選択へ反映した。
  選択状態は色に加えてチェックアイコン（非色）でも提示する
- **お土産詳細ページ**：観光地詳細と同一のエディトリアル意匠・アクション導線（お気に入り／購入済み／
  プラン作成／別のお土産へ）に統一した。お土産データは name / name_kana / description のみのため、
  写真・価格等は表示しない（存在しない情報は提示しない）
- **削除操作**：お気に入り・プランの削除は確認ダイアログをやめ、即時削除＋「元に戻す」トースト（Undo）に変更

### 12.3 依然として別タブに委ねるもの（初版の判断を維持）

- 統計情報：お気に入り・プランタブで確認
- 手動プラン作成・ランダムプラン作成：プランタブに集約
- コンセプトメッセージ：`/about` に集約
